/* La idea de este archivo no es reescribir la rueda
 * ni copiar el código de x11vnc. Esto no tendría 
 * sentido. Mi objetivo es entender el código y documentar
 * un poco el código fuente para extraer ideas.
 * Me encanta el bajo uso de recursos y la idea detrás 
 * de este código. Vamos a aprender
 *
 * De antemano: ver rapidamente las líneas 3760 (initialize_xdisplay_fb)
 * Acá pasan cosas interesantes. En la línea 80 explico donde termina 
 * de "parsear" argumentos.
 */

int main(int argc, char* argv[]) { // Línea 2041 en el archivo real.
                                   // aquí empieza el x11vnc. 
                                   // argc y argv (al igual que en 
                                   // python) se usan para capturar
                                   // los argumentos recibidos por cli

  // Todo este bloque define variables
	int i, len, tmpi;
	int ev, er, maj, min;
	char *arg;
	int remote_sync = 0;
  ...
  /*
   * A partir de esta línea se extraen los argumentos recibidos.
   * por ejemplo el for de más abajo se está verificando si uno de los argumentos 
   * pasados al programa es la cadena "-inetd". 
   *
   * Parámetros de `main`:
   * - `int argc`: Este parámetro representa el número de argumentos que se pasan al 
   *   programa desde la línea de comandos, incluyendo el nombre del programa.
   * - `char* argv[]`: Este es un arreglo de cadenas (punteros a caracteres) que 
   *   contiene los argumentos. `argv[0]` es el nombre del programa, y `argv[1]` 
   *   hasta `argv[argc-1]` son los argumentos proporcionados.
   *
   * Bucle `for`:
   * - El bucle comienza en `i = 1` y se ejecuta mientras `i` sea menor que `argc`. 
   *   Esto significa que se está iterando sobre todos los argumentos pasados al 
   *   programa, excepto el nombre del programa en `argv[0]`.
   *
   * Comparación de Cadenas:
   * - La función `strcmp` se utiliza para comparar la cadena actual `argv[i]` con 
   *   la cadena "-inetd". Si son iguales, `strcmp` devuelve 0, lo que significa que 
   *   la condición `!strcmp(argv[i], "-inetd")` se evalúa como verdadera.
   *
   * Variable `got_inetd`:
   * - Si se encuentra el argumento "-inetd", se establece la variable `got_inetd` en 1 (o verdadero).
   *
   * Así mismo se hace con cada uno de los argumentos recibidos. Este código es super extenso. Sigue
   * hasta la línea ~2170. Para vim se puede ir usando 2100 j
   *
   * En la línea ~3760 hay una función llamada initialize_xdisplay_fb(). 
   */
...
#	define CHECK_ARGC if (i >= argc-1) { \
  ...
  /* Aquí parece finalizar el bloque de argparse 
   * Esto aún no lo tengo claro pero sin duda las próximas
   * líneas no buscan argumentos, más bien dan valores
   * "predeterminados" a algunas variables de entorno y también
   * a partir de aquí parece que se toman las decisiones
  ...
  /*
   * Esta es la parte importante! Aquí es donde se inicializa 
   * la copia del framebuffer de x11. Esto es re importante
   * porque la idea es ver como hacen para capturar el fb de x11
   * Supongo que para Wayland y cada compositor será un mundo
   * nuevo. Pero para ser conservador vamos a empezar por 
   * x11 para aprender. Esta función que se llama acá está en un archivo
   * llamado screen.c
   */ 
